...
public class UserDao {
	...
	public List<User> getUserList(int page) {
		...
		int offset = (page -1) * 10; // page 1 = limit 10 offset 0 / page2 limit 10 offset 10
		Connection conn = getConnection();
	...
	public void insertUser(User user) {
		Connection conn = getConnection();
		String sql = "insert into users values (?, ?, ?, ?, default, default, ?, ?)"; // regDate,,isDelete = default
		try {
			PreparedStatement pstmt = conn.prepareStatement(sql);
	...
	public void updateUser(User user) {
		Connection conn = getConnection();
		String sql = "update users set pwd=?, uname=?, email=?, profile=?, addr=? where uid=?"; // regDate,,isDelete = default
		try {
			PreparedStatement pstmt = conn.prepareStatement(sql);
	...
	public void deleteUser(String uid) { //db는 지우지 않고 isDelete가 1로 바뀜
		Connection conn = getConnection();
		String sql = "update users set isDeleted=1 where uid=?"; // regDate,,isDelete = default
		try {
			PreparedStatement pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, uid);
